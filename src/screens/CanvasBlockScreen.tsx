...
import { getBbox, clipRoadsToPolygon, isPolygonSelfIntersecting, polygonArea, pointInPolygon, getSerpentineOrder, distanceBetween, fetchOverpass, fetchMapFeatures, generateSerpentinePath, bearingBetween, buildComprehensiveQuery, processOverpassData } from '../lib/geo';
...
export default function CanvasBlockScreen({ mapData, onUpdateMapData, onExitToDashboard, onJumpToPreview }: Props) {
  ...
  async function autoDetectAreaForCanvas(targetBlockId?: string, mode: 'append' | 'replace' = 'append') {
    ...
    try {
      ...
      let res: any;
      try {
        res = await fetchMapFeatures(targetBoundary, area, controller.signal);
        if (res.dataSource) {
          alert(`Map Features Auto-Detected using: ${res.dataSource}`);
        }
      } catch (err) {
        setDetectMsg('⚠️ OSM/Google API failed. Try again shortly.');
        setDetectLoading(false);
        return;
      }
